...
#     $ id -u   #gives user ID
#
#  So: if the group name is not the same as the username OR the user id is not
#  greater than 99 (i.e. root or a privileged user), then we are on a
#  local machine (check for yourself), so we set umask 022.
#
#  Conversely, if the default group name is the same as the username
